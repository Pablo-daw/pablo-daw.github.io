// Boton de creación de los botones para los videos

  var miVideo = document.getElementById("miVideo");

  function botonPlay(elemento) {
      elemento.play();
  }

  function botonPause(elemento) {
      elemento.pause();
  }

  function botonStop(elemento) {
      elemento.currentTime = 0;
      elemento.pause();
  }

  function botonRebobinar(elemento) {
      elemento.currentTime = 0;
  }
...
var buttonEnglish, buttonDeutsch, buttonSpanish;
...
window.onload = function () {
    ...
    trackElems = document.querySelectorAll("#all video track");
    ...
    buttonSpanish = document.querySelector("#buttonSpanish");
...
function loadTranscript(lang) { // clear current transcript
    clearTranscriptDiv();
    ...
    for (var i = 0; i < tracks.length; i++) { // current track
        var track = tracks[i];
        ...
        if ((track.language === lang) && (track.kind !== "chapters")) {
            ...
            if (trackAsHtmlElem.readyState === 2) { // the track has already been loaded
                displayCues(track);
            } else {
...
function getVoices(speech) { // takes a text content and check if there are voices
    ...
    while (pos != -1) {
        ...
        voices.push({'voice': voice, 'text': text});
