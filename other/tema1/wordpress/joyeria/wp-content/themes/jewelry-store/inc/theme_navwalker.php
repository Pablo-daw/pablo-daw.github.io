...
class jewelry_store_bootstrap_navwalker extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( strcasecmp( $item->attr_title, 'divider' ) == 0 && $depth >= 1 ) {
            ...
            } elseif( in_array( 'jewelry_store-mm-heading', $item->classes ) && ( $item->url === '#' ) ) {
            	$item_output .= '<span class="mm-heading-wrapper">';
            } elseif( in_array( 'jewelry_store-mm-heading', $item->classes ) ) {
			...
			if( ! empty( $item->description ) && ( $item->description !== ' ' ) && $depth >= 1 ) {
				$item_output .= '<li class="jewelry_store-mm-description">' . $item->description;
			}
	...
	public static function fallback( $args ) {
		if ( current_user_can( 'manage_options' ) ) {
			...
			$fb_output .= '>';
			$fb_output .= '<li class="menu-item"><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '">'.__('Add a menu','jewelry-store').'</a></li>';
			$fb_output .= '</ul>';
